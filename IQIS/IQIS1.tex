...
\begin{description}[leftmargin=0cm]
    \item[국소성{\footnotesize Locality}.] 
        ...
        공간에서 조각 하나의 상태를 갱신할 때는 그 부분 주변의 지식만
        요구되어야 할 것이다. 이에 적절한 모델은 콘웨이의
        생명게임{\footnotesize Game Of Life}이다. 계{\footnotesize system}에 
        일으키는 변화는 계에 영향을 미칠 수도 있다. 하지만 각 칸은 오직 가장
        가까운 주변부와 상호작용할 뿐이다. 그리하여 그 변화는 오직 특정 속도로
        전파한다.
\end{description}
...
전부 사실상 시간을 역행한다.\break

\begin{description}[leftmargin=0cm]
    \item[국소적 실재론{\footnotesize Local Realism}.] 멀리 떨어진 사건에 대한
        지식의 업데이트라면 전부 무작위 변수의 상관관계로 설명할 수 있다는
        원리다. 당신은 오스틴, 친구는 샌프란시스코에 있다고 하자. 두 명 모두
        ...
        헤드라인을 학습하는 바로 그 순간 즉각 친구의 신문도 동일한 헤드라인을
        지닌다고 알 수 있다.
        \end{description}

\newpage
\hfill\parbox[t]{9cm}{\textcolor{gray}{\slshape어떤 대중과학 기사는 한 입자의
스핀을 측정하면 즉각 은하계 이면에 있는 또 다른 입자의 스핀을 알 수 있는
방법을 논한다. 다만 별다른 추가 언급 없이는 신문 사례와 다를 바 없다.
게다가 국소적 실재론과 100\% 호환되는 것으로 보인다!}}
...
\begin{description}[leftmargin=0cm]
    \item[처치-튜링 논제{\footnotesize Chuch-Turing Thesis}.] 처치-튜링 논제란
        ...
        시뮬레이션 가능하다고 진술이다. 처치와 튜링은 이 진술을
        계산{\footnotesize computation}에 대한 정의로 이해했다. 그러나 그 대신
        처치-튜링 논제를 물리 세계에 대한 반증가능한 주장으로 받아들일 수 있다.
        처치-튜링 논제를 전체 우주가 일종의 초대형 비디오 게임이라는 발상으로
        이해하는 것이다. 온갖 종류의 복잡한 것들, 쿼크와 블랙홀, 뭐든 있다.
        다만 종국에는 이들을 컴퓨터로 시뮬레이션할 수 있어야 한다. \textbf{확장
        처치-튜링 논제}는 더 나아간다. 바로 디지털 컴퓨터를 통한 
        실재{\footnotesize reality}의 시뮬레이션에는 시간과 공간과 여타
        계산 자원에 대해 다항{\footnotesize polynomial}의 (즉 선형이나
        이차의) 증가가 최대라는 것이다.
\end{description}
...
\begin{itemize}[label=$\blacktriangleright$]
    \item 확률을 사용하기는 할 것이다. 다만 확률의 \circemph{연산}을 아예
        달리할 것이다. 그리고 단조의 공리를 위배할 것이다. 즉 한 사건이
    ...
    \item 국소성은 놔둔다. 그러나 \circemph{국소적 실재론}은 타도하겠다. 이 두
        ...
        극적으로 묘사하리다!
    \item 앞으로 확인하겠지만 처치-튜링 논제는 양자역학의 후광 밑이라도
        상태가 좋아 보인다. 그러나 확장 처치-튜링 논제는 거짓으로
        보인다. 양자컴퓨팅이 그 반례로서 이글거리며 서 있다. 아마
        우리의 물리학 법칙들이 허용하는 단 하나의 반례일지도 모른다. 그런데도
        이야기한 김에 덧붙이자면 양자 버전으로 확장 처치-튜링 논제를
        정식화할 수도 있다. 이 양자 버전은 오늘날의 지식에 한해 아직 참이다.
\end{itemize}
...
에너지가 필요하겠다. 그리고 그만큼의 에너지를 한 장소에 지니면 당신은 블랙홀로
붕괴한다. 별로 재미없다!}}
